...
import { buildConsignmentNoteSchema } from '@/config/document-editor';

const SECTIONS: Array<{ key: keyof PDFLayoutConfig; label: string }> = [
  { key: 'showClientData', label: 'Datos del consignante' },
  { key: 'showVehicleDetails', label: 'Datos del vehículo' },
  { key: 'showFinancialDetails', label: 'Detalles de la consignación' },
  { key: 'showSignatures', label: 'Firmas' },
];

interface ConsignmentNoteViewerProProps {
  isOpen: boolean;
  onClose: () => void;
  documentId: number;
...
const ConsignmentNoteViewerPro: React.FC<ConsignmentNoteViewerProProps> = ({
  isOpen,
  onClose,
  documentId,
  autoDownload,
  ...
  useAutoDownloadPdf(autoDownload, pdfBlob, `consignacion_${documentId}.pdf`, onClose);
    ...
    const fetchIds = async () => {
      ...
      try {
        ...
        const { data: consData, error: consError } = await supabase
          .from('vehicles_consignments').select('id').eq('document_id', documentId).maybeSingle();
        if (!consError && consData) setConsignmentId(consData.id);
      } catch (error) {
  ...
  const { data: documentData, loading: dataLoading, error, refetch } = useConsignmentNoteData(vehicleId || 0, consignmentId || undefined);
  ...
  useEffect(() => {
    ...
    const generatePdf = async () => {
      ...
      try {
        const doc = <ConsignmentNotePDF {...documentData} layoutConfig={layoutConfig} />;
  ...
  const handleAutoFit = useCallback(async () => {
    ...
    try {
      const best = await autoFitToOnePage(
        (config) => <ConsignmentNotePDF {...documentData} layoutConfig={config} />,
        layoutConfig
      );
  ...
  const editablePreview = documentData ? (
    <GenericDocumentHTMLPreview
      schema={buildConsignmentNoteSchema(documentData, layoutConfig)}
  ...
  return (
    <PDFDocumentEditorDialog
      isOpen={isOpen}
      onClose={onClose}
      title={`Contrato de Consignación${documentData ? ` N° ${documentData.documentNumber}` : ''}`}
      layoutConfig={layoutConfig}
      onLayoutConfigChange={setLayoutConfig}
      pdfUrl={pdfUrl}
      pdfBlob={pdfBlob}
...
export default ConsignmentNoteViewerPro;
