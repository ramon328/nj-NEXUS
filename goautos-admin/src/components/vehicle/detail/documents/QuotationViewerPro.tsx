...
import { useQuotationData } from '@/hooks/documents/useQuotationData';
...
import { buildQuotationSchema } from '@/config/document-editor';
...
const SECTIONS: Array<{ key: keyof PDFLayoutConfig; label: string }> = [
  ...
  { key: 'showFinancialDetails', label: 'Detalle de la cotización' },
  { key: 'showTransferValue', label: 'Valor de transferencia' },
  { key: 'showFinancing', label: 'Opciones de financiamiento' },
  { key: 'showSignatures', label: 'Firma' },
];

interface QuotationViewerProProps {
  isOpen: boolean;
  onClose: () => void;
  documentId: number;
...
const QuotationViewerPro: React.FC<QuotationViewerProProps> = ({
  isOpen,
  onClose,
  documentId,
  autoDownload,
  ...
  useAutoDownloadPdf(autoDownload, pdfBlob, `cotizacion_${documentId}.pdf`, onClose);
    ...
    const fetchIds = async () => {
      ...
      try {
        ...
        const { data: quotData, error: quotError } = await supabase
          .from('vehicles_quotations').select('id').eq('document_id', documentId).maybeSingle();
        if (!quotError && quotData) setQuotationId(quotData.id);
      } catch (error) {
  ...
  const { data: documentData, loading: dataLoading, error, refetch } = useQuotationData(vehicleId || 0, quotationId || undefined);
  ...
  useEffect(() => {
    ...
    const generatePdf = async () => {
      ...
      try {
        const doc = <QuotationPDF {...documentData} layoutConfig={layoutConfig} />;
  ...
  const handleAutoFit = useCallback(async () => {
    ...
    try {
      const best = await autoFitToOnePage(
        (config) => <QuotationPDF {...documentData} layoutConfig={config} />,
        layoutConfig
      );
  ...
  const editablePreview = documentData ? (
    <GenericDocumentHTMLPreview
      schema={buildQuotationSchema(documentData, layoutConfig)}
  ...
  return (
    <PDFDocumentEditorDialog
      isOpen={isOpen}
      onClose={onClose}
      title={`Cotización${documentData ? ` N° ${documentData.documentNumber}` : ''}`}
      layoutConfig={layoutConfig}
      onLayoutConfigChange={setLayoutConfig}
      pdfUrl={pdfUrl}
      pdfBlob={pdfBlob}
...
export default QuotationViewerPro;
