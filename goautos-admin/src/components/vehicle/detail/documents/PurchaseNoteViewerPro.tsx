...
import { buildPurchaseNoteSchema } from '@/config/document-editor';

const SECTIONS: Array<{ key: keyof PDFLayoutConfig; label: string }> = [
  { key: 'showClientData', label: 'Datos del vendedor' },
  { key: 'showVehicleDetails', label: 'Datos del vehículo' },
  { key: 'showFinancialDetails', label: 'Detalle de la compra' },
  { key: 'showSignatures', label: 'Firmas' },
];

interface PurchaseNoteViewerProProps {
  isOpen: boolean;
  onClose: () => void;
  documentId: number;
...
const PurchaseNoteViewerPro: React.FC<PurchaseNoteViewerProProps> = ({
  isOpen,
  onClose,
  documentId,
  autoDownload,
  ...
  useAutoDownloadPdf(autoDownload, pdfBlob, `nota_compra_${documentId}.pdf`, onClose);
    ...
    const fetchIds = async () => {
      ...
      try {
        ...
        const { data: docData, error: docError } = await supabase
          .from('vehicles_documents').select('vehicle_id, customer_id, layout_config').eq('id', documentId).single();
        if (docError) throw docError;
        setVehicleId(docData.vehicle_id);
        setSavedDocumentLayoutConfig(docData.layout_config || undefined);

        const { data: purchaseByDoc } = await supabase
          .from('vehicles_purchases').select('id').eq('document_id', documentId).maybeSingle();
        if (purchaseByDoc) {
          setPurchaseId(purchaseByDoc.id);
        } else if (docData.customer_id) {
          const { data: purchaseByCustomer } = await supabase
            .from('vehicles_purchases').select('id')
            .eq('vehicle_id', docData.vehicle_id).eq('customer_id', docData.customer_id)
            .order('created_at', { ascending: false }).limit(1).maybeSingle();
          if (purchaseByCustomer) setPurchaseId(purchaseByCustomer.id);
        }
      } catch (error) {
  ...
  const { data: documentData, loading: dataLoading, error, refetch } = usePurchaseNoteData(vehicleId || 0, purchaseId || undefined);
  ...
  useEffect(() => {
    ...
    const generatePdf = async () => {
      ...
      try {
        const doc = <PurchaseNotePDF {...documentData} layoutConfig={layoutConfig} />;
  ...
  const handleAutoFit = useCallback(async () => {
    ...
    try {
      const best = await autoFitToOnePage(
        (config) => <PurchaseNotePDF {...documentData} layoutConfig={config} />,
        layoutConfig
      );
  ...
  const editablePreview = documentData ? (
    <GenericDocumentHTMLPreview
      schema={buildPurchaseNoteSchema(documentData, layoutConfig)}
  ...
  return (
    <PDFDocumentEditorDialog
      isOpen={isOpen}
      onClose={onClose}
      title={`Nota de Compra${documentData ? ` N° ${documentData.documentNumber}` : ''}`}
      layoutConfig={layoutConfig}
      onLayoutConfigChange={setLayoutConfig}
      pdfUrl={pdfUrl}
      pdfBlob={pdfBlob}
...
export default PurchaseNoteViewerPro;
