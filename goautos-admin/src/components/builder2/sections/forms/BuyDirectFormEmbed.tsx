...
interface BuyDirectFormEmbedProps {
  bgColor?: string;
  textColor?: string;
  accentColor?: string;
  title?: string;
...
export const BuyDirectFormEmbed = ({
  bgColor = '#ffffff',
  textColor = '#111827',
  accentColor = '#3b82f6',
  title = '',
  subtitle = '',
}: BuyDirectFormEmbedProps) => {
  ...
  const FakePriceInput = ({ label }: { label: string }) => (
    <div className="w-full">
      <div className="text-xs mb-1" style={{ color: labelColor }}>{label}</div>
      <div className="h-10 rounded-lg border px-3 flex items-center gap-2" style={{ backgroundColor: inputBg, borderColor: inputBorder }}>
        <span className="text-sm font-medium" style={{ color: subtextColor }}>$</span>
        <span className="text-xs" style={{ color: labelColor }}></span>
      </div>
    </div>
  );
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
          ...
          <div className="rounded-xl shadow-lg p-8 border" style={{ backgroundColor: cardBg, borderColor: cardBorder }}>
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <FakeSelect label="Marca" />
                <FakeSelect label="Modelo" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <FakeInput label="Año" />
                <FakeInput label="Kilometraje" />
              </div>
              <FakeSelect label="Estado" />
              <FakePriceInput label="Precio pedido" />
          ...
          <div className="rounded-xl p-8 border" style={{ backgroundColor: infoBg, borderColor: cardBorder }}>
            ...
            <div className="space-y-5 mb-8">
              {[
                { step: '1', title: 'Envía tus datos', desc: 'Completa el formulario con la información de tu vehículo.' },
                { step: '2', title: 'Evaluamos', desc: 'Nuestro equipo revisa los datos y evalúa tu vehículo.' },
                { step: '3', title: 'Te hacemos una oferta', desc: 'Recibirás una oferta justa y transparente.' },
                { step: '4', title: 'Pago inmediato', desc: 'Si aceptas, realizamos el pago de forma inmediata.' },
              ].map((item, i) => (
...
BuyDirectFormEmbed.craft = {
  displayName: 'BuyDirectFormEmbed',
  props: {
    bgColor: '#ffffff',
    textColor: '#111827',
    accentColor: '#3b82f6',
    title: 'Compra Directa',
    subtitle: '¿Quieres vender tu vehículo de forma rápida y segura? Te hacemos una oferta al instante.',
  },
