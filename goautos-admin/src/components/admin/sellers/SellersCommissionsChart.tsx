...
interface SellersCommissionsChartProps {
  sellersStats: SellerStats[];
  loading: boolean;
}

export const SellersCommissionsChart = ({
  sellersStats,
  loading,
}: SellersCommissionsChartProps) => {
  ...
  const sorted = [...sellersStats]
    .map((stat) => ({
      ...
      commissions: stat.totalCommissions,
      vehiclesSold: stat.vehiclesSold,
    }))
    .sort((a, b) => b.commissions - a.commissions);

  const maxComm = Math.max(...sorted.map((s) => s.commissions), 1);
  ...
  return (
    <div className="rounded-2xl border border-slate-200/60 bg-white">
      ...
      <div className="px-4 py-3">
        <div className="flex items-center gap-2.5">
          <Coins className="w-[18px] h-[18px] text-slate-900" />
          <h3 className="text-[15px] font-semibold text-slate-900">{dv('Comisiones por Vendedor', 'Commissions by Seller')}</h3>
      ...
      {sorted.length === 0 ? (
        ...
        <div className="px-4 pb-4 space-y-2">
          {sorted.map((item) => {
            const pct = maxComm > 0 ? (item.commissions / maxComm) * 100 : 0;
            ...
            return (
              <div key={item.name} className="flex items-center gap-3">
                ...
                <div className="shrink-0 text-right flex items-center gap-2">
                  <span className="text-[13px] font-semibold text-slate-900 tabular-nums">{formatPrice(item.commissions)}</span>
