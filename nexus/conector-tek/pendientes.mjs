...
export function listarPendientes({ userId, empresa, asistido = true } = {}) {
  ...
  return new Promise((resolve) => {
    ...
    // AUTO-LOGIN CON CLICS HUMANOS: el spawn es login-humano en MODO AUTO (reusa si viva,
    // loguea solo con clics humanos si dormida) + lee las pendientes. Solo si el login no
    // pudo entrar por sí mismo (Superclave / rebote) caemos al ASISTIDO (link + PIN).
    ...
    const fin = () => {
      ...
      // El login automático no pudo entrar solo → ASISTIDO con la consulta enganchada.
      if (!p && asistido && process.env.TEK_SIN_ASISTIDO !== '1' && (seguridad || puerta.loginNecesitaHumano(resultado))) {
        ...
        return resolve({
          ...
          nota: 'El login automático no pudo entrar solo. Te abrí el login para que entres vos; apenas entres leo las pendientes.',
        })
      }
