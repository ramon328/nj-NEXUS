...
let ctx = null
let activa = null
let ultimoPdf = null   // último PDF descargado por el navegador (ver /ultimo-pdf)

function navegadorVivo() {
  // En modo visible la ventana se puede CERRAR (usuario/sistema) y el contexto queda
  // muerto → "Target page, context or browser has been closed". Detectarlo para relanzar.
  if (!ctx) return false
  try {
    const b = ctx.browser()
...
async function asegurarNavegador() {
  if (navegadorVivo()) return
  // Si había un contexto muerto (ventana cerrada), lo descartamos y relanzamos.
  if (ctx) { try { await ctx.close() } catch { /* ya cerrado */ } ctx = null; activa = null }
  // Motor: Google Chrome (NO Brave). Brave rompe GoAutos (portal.goauto.cl) por sus
  // shields/privacidad; en Chrome y Safari funciona. channel:'chrome' usa el Chrome
  // instalado del sistema. viewport:null + --start-maximized para ver bien en modo visible.
  ...
  // Captura cualquier PDF que el navegador descargue (ej. la VISTA PREVIA del DTE que
  // el SII genera dentro de un iframe con POST a mipePreView.cgi). Así podemos mandar
  // el PDF REAL del SII en vez de una captura de pantalla. Se expone en /ultimo-pdf.
  ctx.on('response', async (resp) => {
    try {
      ...
      const ruta = join('/tmp', `nexus-sii-pdf-${Date.now()}.pdf`)
      writeFileSync(ruta, buf)
      ultimoPdf = { ruta, url: resp.url(), ts: Date.now(), bytes: buf.length }
      log(`  ⇩ PDF capturado del SII: ${ruta} (${buf.length} bytes)`)
    } catch { /* no romper la navegación por esto */ }
  })
  // Si cierran la ventana, marcamos el contexto como muerto → se relanza al próximo uso.
  ctx.on('close', () => { log('Contexto del navegador cerrado (ventana). Se relanzará al próximo uso.'); ctx = null; activa = null })
  activa = ctx.pages()[0] || await ctx.newPage()
  log(`Navegador iniciado: Chrome (channel) · visible=${VISIBLE}`)
}
...
// Escribe un valor en un campo, SIRVA O NO como <select>. Necesario para el SII:
// si el receptor ya tiene direcciones conocidas, el SII convierte "Direccion" en un
// <select> con SOLO esas (no deja escribir una nueva) → acá inyectamos la opción y la
// seleccionamos, que es equivalente al <input> de texto libre que el propio SII usa
// cuando no conoce direcciones. Body: { selector, valor, hidden? }.
app.post('/forzar-valor', async (req, res) => {
  try {
    const p = await pagina()
    const { selector, valor, hidden } = req.body || {}
    if (!selector) return res.status(400).json({ error: 'Falta selector' })
    const r = await p.evaluate(({ sel, val, hid }) => {
      const el = document.querySelector(sel)
      if (!el) return { ok: false, motivo: 'campo no existe' }
      const tag = el.tagName.toUpperCase()
      if (tag === 'SELECT') {
        let op = [...el.options].find((o) => (o.value || '').trim() === String(val).trim())
        if (!op) { op = new Option(val, val, true, true); el.add(op) }
        el.value = op.value
      } else {
        el.value = val
      }
      if (hid) { const h = document.querySelector(hid); if (h) h.value = val }
      el.dispatchEvent(new Event('change', { bubbles: true }))
      return { ok: true, tag, valor: el.value }
    }, { sel: String(selector), val: String(valor ?? ''), hid: hidden ? String(hidden) : null })
    log(`POST /forzar-valor: ${selector} -> ${JSON.stringify(r)}`)
    res.json(r)
  } catch (e) { res.status(500).json({ error: e.message }) }
})

// Último PDF que descargó el navegador (ej. la vista previa del DTE del SII).
// ?olvidar=1 lo borra de memoria para no reusar uno viejo por error.
app.get('/ultimo-pdf', (req, res) => {
  const p = ultimoPdf
  if (req.query?.olvidar === '1') ultimoPdf = null
  res.json({ ok: Boolean(p), ...(p || {}) })
})

// Selecciona una opción de un <select>. Body: { selector, valor?, etiqueta? }.
// Elige por value (valor) o por texto visible (etiqueta).
app.post('/seleccionar', async (req, res) => {
  try {
    const p = await pagina()
    const { selector, valor, etiqueta } = req.body || {}
    if (!selector) return res.status(400).json({ error: 'Falta selector' })
    await p.waitForSelector(String(selector), { state: 'visible', timeout: 15000 })
    const arg = valor != null ? { value: String(valor) } : { label: String(etiqueta) }
    const elegido = await p.selectOption(String(selector), arg, { timeout: 15000 })
    log(`POST /seleccionar: ${selector} -> ${JSON.stringify(arg)}`)
    res.json({ ok: true, elegido })
  } catch (e) { res.status(500).json({ error: e.message }) }
})

// Lee los CAMPOS de formulario de la página (name/id/tipo) para mapear un form
// sin adivinar selectores. Devuelve inputs, selects (con opciones) y botones.
app.get('/campos', async (_req, res) => {
  try {
    const p = await pagina()
...
// Inyecta cookies de una sesión ya iniciada (ej. SII) en el contexto del navegador,
// para quedar logueado SIN re-loguear (evita bloqueos por intentos). Body:
// { cookies: {nombre:valor, ...}, domain: ".sii.cl" }. Solo localhost lo usa.
app.post('/inyectar-cookies', async (req, res) => {
  try {
    await asegurarNavegador()
    const dict = req.body?.cookies || {}
    const domain = String(req.body?.domain || '.sii.cl')
    const cookies = Object.entries(dict).map(([name, value]) => ({
      name, value: String(value), domain, path: '/',
    }))
    if (!cookies.length) return res.status(400).json({ error: 'Sin cookies' })
    await ctx.addCookies(cookies)
    log(`POST /inyectar-cookies: ${cookies.length} cookies en ${domain}`)
    res.json({ ok: true, inyectadas: cookies.length, domain })
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
// Reintenta una acción de página (escribir/click/subir) que suele fallar en el
// PRIMER intento porque la SPA todavía no terminó de renderizar el elemento.
// Es justo el caso de "falló, reintento → funciona": ahora lo hace solo. Solo
// reintenta ante fallos transitorios (timeout / elemento no listo); un error
// real (selector inexistente de verdad) se propaga tras agotar los intentos.
async function conReintentos(fn, { intentos = 3, delays = [800, 2000, 4000], etiqueta = '' } = {}) {
  let ultimoError
  for (let i = 1; i <= intentos; i++) {
    try {
      ...
      ultimoError = e
      const transitorio = /timeout|not (visible|stable|enabled|attached)|detached|Execution context|Target (closed|page)|element is not|intercepts pointer/i.test(e.message || '')
      if (!transitorio || i === intentos) break
      const d = delays[i - 1] ?? delays[delays.length - 1]
      log(`  → reintento ${i + 1}/${intentos} en ${d}ms (${etiqueta}): ${String(e.message).split('\n')[0]}`)
...
app.post('/escribir', async (req, res) => {
  try {
    ...
    await conReintentos(async () => {
      // Espera a que el campo exista y sea visible ANTES de escribir (SPA tardía).
...
app.post('/subir-archivo', async (req, res) => {
  try {
    ...
    await conReintentos(
      () => p.setInputFiles(String(selector), lista, { timeout: 20000 }),
      { etiqueta: String(selector) }
    )
...
app.post('/click', async (req, res) => {
  try {
    ...
    await conReintentos(async () => {
      if (selector) {
        // Espera a que el elemento esté visible antes de clickear (SPA tardía).
        await p.waitForSelector(selector, { state: 'visible', timeout: 15000 })
        await p.click(selector, { timeout: 15000 })
      } else {
        const loc = p.getByText(texto, { exact: false }).first()
...
app.get('/captura', async (req, res) => {
  try {
    const p = await pagina()
    const buf = await p.screenshot({ type: 'png', fullPage: req.query.full === '1' })
...
app.get('/leer', async (req, res) => {
  try {
    ...
    // ?html=1 devuelve el HTML del DOM (para mapear formularios / hallar embeds de PDF)
    if (req.query?.html === '1') {
      const html = (await p.evaluate(() => document.documentElement?.outerHTML || '')).slice(0, 200000)
      log(`  ✓ leído HTML ${html.length} caracteres`)
      return res.json({ ok: true, url: p.url(), html, caracteres: html.length })
    }
