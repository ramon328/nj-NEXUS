...
async function ejecutar(nombre, input, ctx = {}) {
  try {
    ...
    if (nombre === 'compra') {
      ...
      // El PAGO (paso 2) sale de ANA CLARA vía transferencia masiva (tek_masiva). Se
      // pre-arma con el vendedor y el precio del expediente; queda listo para que el
      // modelo lo suba con tek_masiva (una sola transferencia = lote de 1). NUNCA se
      // autoriza solo: sube "Por Autorizar" y lo libera un humano con Superclave.
      const pagoSugerido = (e) => ({
        ...
        via: 'tek_masiva (sube el pago masivo desde ANA CLARA; queda Por Autorizar, lo libera un humano)',
      })
      ...
      return JSON.stringify({
        ...
        instruccion: `Tablero de la compra de ${patente}. Muéstralo ordenado, di qué falta ("necesito") y cuál es el siguiente paso (${sig || 'ninguno, ya está completo'}). Para el PAGO usa tek_masiva con el beneficiario y monto de "pago" (sale de ANA CLARA; queda Por Autorizar, lo libera un humano).`,
      })
    }
