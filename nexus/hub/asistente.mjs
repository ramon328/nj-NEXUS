    ...
    if (nombre === 'crear_contrato') {
      ...
      try {
        ...
        if (accion === 'vendedor') {
          ...
          if (!input.confirmar) {
            return JSON.stringify({
              ...
              ojo_documentos: esEmpresa && !docsEmpresa.eRutSii
                ? '⚠️ FALTA EL e-RUT DEL SII. Es el que más se olvida porque la gente lo manda como FOTO de la app del SII, no como PDF. Pídeselo antes de confirmar: "mándame la foto del e-RUT de la empresa". Si igual quieren seguir sin él, se puede — no bloquea la firma —, pero avísales que después lo van a pedir.'
                : null,
        ...
        if (['siguiente', 'permiso', 'comprador', 'contraparte', 'firma_comprador', 'impuestos'].includes(accion)) {
          ...
          if (accion === 'siguiente') {
            const pedir = {
              vendedor: 'los DATOS DEL VENDEDOR (el titular del auto, o sea a QUIEN se le está comprando). PRIMERO fíjate en el titular que trae el informe/CAV: si es una razón social (SpA, SA, Ltda, EIRL) o el RUT es sobre 50 millones, es una EMPRESA y hay que pedir: domicilio social (calle + número + comuna), el REPRESENTANTE LEGAL completo (nombres, apellidos, RUT de persona, email y teléfono), que es quien firma, Y LOS 4 DOCUMENTOS DE LA SOCIEDAD: e-RUT del SII (la foto de la app sirve), escritura de constitución, vigencia de sociedad y vigencia de poderes. Pídelos SIEMPRE los cuatro: el e-RUT es el que más se olvida porque llega como foto y no como PDF. Del representante legal y del notario NO preguntes lo que ya diga la escritura o la vigencia de poderes: léelas — la razón social y el RUT de la empresa ya los tienes del informe, no los preguntes de nuevo. Si es persona natural: nombres, apellidos, RUT, email, teléfono y dirección con comuna. Después llama accion:"vendedor".',
              contraparte: 'los DATOS DE LA CONTRAPARTE — la EMPRESA o persona A QUIEN se le está comprando el auto (o a quien se le vende, si Mallorca vende). Si el titular del informe es una razón social, trátala como EMPRESA: razón social y RUT ya los tienes del informe; pide el domicilio social (calle + número + comuna), el REPRESENTANTE LEGAL (nombres, apellidos, RUT de persona, email, teléfono) y LOS 4 DOCUMENTOS: e-RUT del SII (vale la foto de la app), escritura de constitución, vigencia de sociedad y vigencia de poderes. ⛔ Del lado de ANA CLARA SPA no preguntes NADA: va automático. Después llama accion:"contraparte".',
