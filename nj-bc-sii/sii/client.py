...
class SiiClient:
    ...
    def __init__(self, throttle: Throttle, max_retries: int = 3, timeout: int = 60):
        ...
        # Si SII_PROXY está definido (p.ej. en Render), todo el tráfico al SII
        # sale por una IP residencial chilena → evita el bloqueo por IP.
        proxies = requests_proxies()
        if proxies:
            self.session.proxies.update(proxies)
            log.info("Sesión SII enrutada por proxy (IP chilena).")
    ...
    def promover_dominio_sii(self) -> None:
        ...
        SIN sesión y el SII responde 401. Esto la promueve a .sii.cl, igual que
        carpeta.py ya hace para Playwright. Llamar tras login/cargar cookies,
        antes de cualquier petición a www4.
        """
        pares = [
            (c.name, c.value)
    ...
    def exp_pasada(self, margen_seg: int = 60) -> bool:
        """True si la sesión del SII ya EXPIRÓ según su cookie de expiración
        (NETSCAPE_LIVEWIRE.exp, hora de Chile).

        CLAVE: el home de misiir sigue cargando aunque la sesión esté vencida, así
        que is_authenticated da falsos positivos y se reusa una sesión muerta → los
        servicios (RCV, F22…) responden 401 ('no encuentra datos'). Este chequeo
        local (sin red) detecta la expiración real para forzar un re-login.
        """
        exp_raw = self.session.cookies.get_dict().get("NETSCAPE_LIVEWIRE.exp")
        if not exp_raw:
            return False  # sin dato → que decida is_authenticated
        s = str(exp_raw)[:14]
        if len(s) == 12:
            s += "00"
        try:
            ...
            exp = datetime.strptime(s, "%Y%m%d%H%M%S").replace(
                tzinfo=ZoneInfo("America/Santiago")
            )
            return (exp - datetime.now(timezone.utc)).total_seconds() <= margen_seg
