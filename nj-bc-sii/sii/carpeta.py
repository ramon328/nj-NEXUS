...
from .proxy import chromium_args, playwright_proxy
...
def generar_oficial(client: SiiClient, rut: str, dest_rut: str, dest_nombre: str,
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch(headless=True, proxy=playwright_proxy(), args=chromium_args())
...
def obtener_datos(client: SiiClient, rut: str) -> dict:
    ...
    with sync_playwright() as p:
        ...
        page.goto(PORTAL, wait_until="networkidle", timeout=60000)
        page.wait_for_timeout(3500)
        if "BIENVENIDO" not in page.inner_text("body").upper():
