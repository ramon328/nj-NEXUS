...
def genera_propuesta_f29_pdf(filas_por_periodo: list, empresa: str, rut: str,
                             out_path: Path) -> bool:
    """Propuesta F29 (IVA mensual) calculada desde el RCV — como hace LibreDTE.

    `filas_por_periodo` = lista de {periodo, operacion, neto, exento, iva, total, docs}.
    Por cada periodo: Débito Fiscal (IVA de VENTAS) − Crédito Fiscal (IVA de
    COMPRAS) = IVA determinado. Positivo → IVA a pagar; negativo → remanente
    (crédito a favor del mes siguiente).

    ⚠️ Es una ESTIMACIÓN con los datos del RCV: NO incluye PPM, retenciones ni
    remanente arrastrado de meses anteriores. No reemplaza la declaración oficial.
    """
    if not filas_por_periodo:
        return False

    periodos = sorted({str(x.get("periodo", "")) for x in filas_por_periodo})

    def _iva(per: str, op: str) -> int:
        return sum(int(x.get("iva") or 0) for x in filas_por_periodo
                   if str(x.get("periodo")) == per and x.get("operacion") == op)

    doc = SimpleDocTemplate(str(out_path), pagesize=A4, topMargin=18 * mm,
                            bottomMargin=15 * mm, leftMargin=15 * mm, rightMargin=15 * mm)
    H1, H2, P = ESTILOS["Title"], ESTILOS["Heading2"], ESTILOS["BodyText"]
    el = [Paragraph("PROPUESTA F29 — IVA MENSUAL", H1),
          Paragraph(f"{empresa} — RUT {rut}", H2),
          Paragraph("Estimación del IVA a pagar calculada con el RCV "
                    "(débito de ventas − crédito de compras).", P),
          Spacer(1, 8)]

    data = [["Periodo", "Débito (IVA ventas)", "Crédito (IVA compras)",
             "IVA determinado", "Estado"]]
    t_deb = t_cre = 0
    for per in periodos:
        deb, cre = _iva(per, "VENTA"), _iva(per, "COMPRA")
        det = deb - cre
        estado = "A pagar" if det > 0 else ("Remanente" if det < 0 else "—")
        legible = periodo_legible(per) if len(per) >= 6 else per
        data.append([legible, clp(deb), clp(cre), clp(abs(det)), estado])
        t_deb += deb
        t_cre += cre
    t_det = t_deb - t_cre
    data.append(["TOTAL", clp(t_deb), clp(t_cre), clp(abs(t_det)),
                 "A pagar" if t_det > 0 else ("Remanente" if t_det < 0 else "—")])
    el.append(_tabla(data, [34 * mm, 36 * mm, 38 * mm, 32 * mm, 24 * mm],
                     con_total=True))
    el.append(Spacer(1, 10))
    el.append(Paragraph(
        "Nota: estimación con base en el RCV. No incluye PPM, retenciones ni "
        "remanente de crédito de meses anteriores. No reemplaza la declaración "
        "oficial del F29 en el SII.", P))
    doc.build(el)
    return True


def genera_propuesta_f29_oficial_pdf(por_periodo: list, empresa: str, rut: str,
                                     out_path: Path) -> bool:
    """Propuesta F29 PRECARGADA por el SII: códigos por periodo (genérico).

    `por_periodo` = [{periodo, codigos: [{codigo, glosa, valor}, …]}, …].
    Defensivo con los nombres de campo (el formato no se pudo verificar en vivo).
    """
    bloques = [b for b in (por_periodo or []) if b.get("codigos")]
    if not bloques:
        return False
    doc = SimpleDocTemplate(str(out_path), pagesize=A4, topMargin=18 * mm,
                            bottomMargin=15 * mm, leftMargin=15 * mm, rightMargin=15 * mm)
    H1, H2, P = ESTILOS["Title"], ESTILOS["Heading2"], ESTILOS["BodyText"]
    el = [Paragraph("PROPUESTA F29 PRECARGADA (SII)", H1),
          Paragraph(f"{empresa} — RUT {rut}", H2),
          Paragraph("Códigos precargados por el SII para declarar el F29. "
                    "(El IVA a pagar estimado está en 'Propuesta F29 / IVA a pagar'.)", P),
          Spacer(1, 8)]
    for blk in bloques:
        per = str(blk.get("periodo", ""))
        el.append(Paragraph(periodo_legible(per) if len(per) >= 6 else per, H2))
        data = [["Código", "Glosa", "Valor"]]
        for x in blk["codigos"]:
            if not isinstance(x, dict):
                continue
            cod = x.get("codigo") or x.get("cod") or x.get("codigoFormulario") or ""
            glosa = x.get("glosa") or x.get("descripcion") or x.get("nombre") or ""
            val = (x.get("valor") or x.get("monto") or x.get("valorPropuesto")
                   or x.get("valorCodigo") or "")
            val_txt = clp(val) if str(val).lstrip("-").isdigit() else str(val)
            data.append([str(cod), str(glosa)[:60], val_txt])
        if len(data) > 1:
            el.append(_tabla(data, [22 * mm, 96 * mm, 36 * mm]))
            el.append(Spacer(1, 10))
    doc.build(el)
    return True


def consolidar_financiero(rcv_tot: list) -> list:
    """Reorganiza los totales RCV (por periodo+operación) a un consolidado por
    periodo con ventas/compras/IVA — pensado para cálculos y automatización."""
    out = []
    for per in sorted({str(x.get("periodo", "")) for x in rcv_tot}):
        v = next((x for x in rcv_tot
                  if str(x.get("periodo")) == per and x.get("operacion") == "VENTA"), {})
        co = next((x for x in rcv_tot
                   if str(x.get("periodo")) == per and x.get("operacion") == "COMPRA"), {})
        deb, cre = int(v.get("iva") or 0), int(co.get("iva") or 0)
        out.append({
            "periodo": per,
            "ventas_neto": int(v.get("neto") or 0),
            "ventas_exento": int(v.get("exento") or 0),
            "ventas_docs": int(v.get("docs") or 0),
            "iva_debito": deb,
            "compras_neto": int(co.get("neto") or 0),
            "compras_exento": int(co.get("exento") or 0),
            "compras_docs": int(co.get("docs") or 0),
            "iva_credito": cre,
            "iva_determinado": deb - cre,
        })
    return out


def genera_resumen_financiero_pdf(consolidado: list, empresa: str, rut: str,
                                  out_path: Path) -> bool:
    """Resumen financiero consolidado por periodo (ventas, compras, IVA)."""
    if not consolidado:
        return False
    doc = SimpleDocTemplate(str(out_path), pagesize=A4, topMargin=18 * mm,
                            bottomMargin=15 * mm, leftMargin=12 * mm, rightMargin=12 * mm)
    H1, H2, P = ESTILOS["Title"], ESTILOS["Heading2"], ESTILOS["BodyText"]
    el = [Paragraph("RESUMEN FINANCIERO", H1),
          Paragraph(f"{empresa} — RUT {rut}", H2),
          Paragraph("Datos consolidados del RCV por periodo (para conciliación, "
                    "cálculos y automatización con sistemas externos).", P),
          Spacer(1, 8)]
    data = [["Periodo", "Ventas neto", "IVA débito", "Compras neto",
             "IVA crédito", "IVA a pagar"]]
    tv = td = tc = tcr = 0
    for x in consolidado:
        per = str(x["periodo"])
        legible = periodo_legible(per) if len(per) >= 6 else per
        data.append([legible, clp(x["ventas_neto"]), clp(x["iva_debito"]),
                     clp(x["compras_neto"]), clp(x["iva_credito"]),
                     clp(x["iva_determinado"])])
        tv += x["ventas_neto"]; td += x["iva_debito"]
        tc += x["compras_neto"]; tcr += x["iva_credito"]
    data.append(["TOTAL", clp(tv), clp(td), clp(tc), clp(tcr), clp(td - tcr)])
    el.append(_tabla(data, [28 * mm, 30 * mm, 26 * mm, 30 * mm, 26 * mm, 26 * mm],
                     con_total=True))
    doc.build(el)
    return True
